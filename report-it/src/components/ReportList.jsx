...
export default function ReportList({ data }) {
  const exportCSV = () => {
    ...
    const rows = data.map(r => [
      ...
      r.customer,
      r.priority,
      // If workDone is an array, join it; otherwise, just use it
    ...
    <div className="bg-white rounded-xl shadow-lg p-8 mt-5">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div className="mb-4 sm:mb-0 text-center">
          ...
          <p className="text-gray-600 mt-1 text-center">Submitted field reports</p>
        </div>
        <button
          onClick={exportCSV}
      ...
      {data.length === 0 ? (
        <div className="text-center py-12 bg-blue-50 rounded-xl">
          ...
          <p className="text-gray-600">Start by submitting your first field report! 🚀</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-blue-50">
              <tr>
                <th className="px-4 py-3 text-left text-sm font-medium text-blue-800">Priority</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-blue-800">Project #</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-blue-800">Customer</th>
                <th className="px-4 py-3 text-left text-sm font-medium text-blue-800">Work Done</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.map((r, i) => (
                <tr key={i} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 text-sm text-gray-700 font-medium">{r.priority}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">{r.projectNumber}</td>
